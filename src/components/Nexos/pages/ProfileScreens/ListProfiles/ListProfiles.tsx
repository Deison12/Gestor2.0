...
export const columns: any[] = [
  ...
  },
  {
    ...
    Cell: ({ row }: { row: any }) => {
      ...
      return (
        <Link
          to={`${process.env.PUBLIC_URL}/nexos/editprofile`}
          state={row.original}
        >
...
const ListProfiles = () => {
  ...
  const fetchData = async () => {
    try {
      const profiles = await getAllData("api/users/profiles");
      setData(profiles);
    } catch (error) {
  ...
  return (
    <div>
      {/* Breadcrumb */}
      <BreadCrumb
        items={["inicio", "perfiles"]}
        baseURL={["inicio", "listprofiles"]}
      />

      {/* Component Title */}
      <TitleComponent title={"LISTA DE PERFILES"} />
...

      {
        ...
        !error && (
          <BasicDataTable
            ...
            data={data}
            isLoading={isLoading}
            addButtonLink={`${process.env.PUBLIC_URL}/nexos/createprofile`}
          />
        )
      }
    </div>
